...
var PostgresDatabaseAdapter = class extends import_core.DatabaseAdapter {
  pool;
  maxRetries = 3;
  baseDelay = 1e3;
  // 1 second
  ...
  constructor(connectionConfig) {
    ...
    this.pool.on("error", (err) => {
      import_core.elizaLogger.error("Unexpected pool error", err);
    ...
    for (let attempt = 1; attempt <= this.maxRetries; attempt++) {
      try {
        ...
        if (attempt < this.maxRetries) {
          ...
          import_core.elizaLogger.error("Max retry attempts reached:", {
            error: error instanceof Error ? error.message : String(error),
  ...
  async handlePoolError(error) {
    import_core.elizaLogger.error("Pool error occurred, attempting to reconnect", {
      error: error.message
    });
    try {
      await this.pool.end();
      ...
      import_core.elizaLogger.success("Pool reconnection successful");
    } catch (reconnectError) {
      import_core.elizaLogger.error("Failed to reconnect pool", {
  ...
  async validateVectorSetup() {
    try {
      ...
      if (!hasVector) {
        import_core.elizaLogger.error("Vector extension not found in database");
        return false;
      }
      return true;
    } catch (error) {
      import_core.elizaLogger.error("Failed to validate vector extension:", {
        error: error instanceof Error ? error.message : String(error)
      });
      return false;
    }
  ...
  async init() {
    ...
    try {
      ...
      const embeddingConfig = (0, import_core.getEmbeddingConfig)();
      if (embeddingConfig.provider === import_core.EmbeddingProvider.OpenAI) {
      ...
      } else if (embeddingConfig.provider === import_core.EmbeddingProvider.Ollama) {
        await client.query("SET app.use_openai_embedding = 'false'");
        await client.query("SET app.use_ollama_embedding = 'true'");
        await client.query("SET app.use_gaianet_embedding = 'false'");
      } else if (embeddingConfig.provider === import_core.EmbeddingProvider.GaiaNet) {
        await client.query("SET app.use_openai_embedding = 'false'");
        await client.query("SET app.use_ollama_embedding = 'false'");
      ...
      if (!rows[0].exists || !await this.validateVectorSetup()) {
        import_core.elizaLogger.info(
          "Applying database schema - tables or vector extension missing"
        );
        const schema = import_fs.default.readFileSync(
          import_path.default.resolve(__dirname, "../schema.sql"),
          "utf8"
        );
  ...
  async testConnection() {
    ...
    try {
      ...
      import_core.elizaLogger.error("Database connection test failed:", error);
      throw new Error(
  ...
  async cleanup() {
    try {
      await this.pool.end();
      import_core.elizaLogger.info("Database pool closed");
    } catch (error) {
      import_core.elizaLogger.error("Error closing database pool:", error);
    }
  }
  ...
  async getAccountById(userId) {
    return this.withDatabase(async () => {
      ...
      if (rows.length === 0) {
        import_core.elizaLogger.debug("Account not found:", { userId });
        return null;
      }
  ...
  async createAccount(account) {
    return this.withDatabase(async () => {
      try {
        const accountId = account.id ?? (0, import_uuid.v4)();
        await this.pool.query(
        ...
        import_core.elizaLogger.debug("Account created successfully:", {
          accountId
        });
        return true;
      } catch (error) {
        import_core.elizaLogger.error("Error creating account:", {
          error: error instanceof Error ? error.message : String(error),
  ...
  async getActorById(params) {
    return this.withDatabase(async () => {
      ...
      import_core.elizaLogger.debug("Retrieved actors:", {
      ...
      return rows.map((row) => {
        try {
          ...
          import_core.elizaLogger.warn("Failed to parse actor details:", {
            actorId: row.id,
            error: error instanceof Error ? error.message : String(error)
          });
          return {
      ...
      import_core.elizaLogger.error("Failed to get actors:", {
        roomId: params.roomId,
        error: error.message
      });
      throw error;
  ...
  async createMemory(memory, tableName) {
    return this.withDatabase(async () => {
      var _a, _b, _c;
      import_core.elizaLogger.debug("PostgresAdapter createMemory:", {
        memoryId: memory.id,
  ...
  async getMemories(params) {
    ...
    return this.withDatabase(async () => {
      ...
      import_core.elizaLogger.debug("Fetching memories:", {
        roomId: params.roomId,
        tableName: params.tableName,
        unique: params.unique,
        agentId: params.agentId,
  ...
  async updateGoal(goal) {
    return this.withDatabase(async () => {
      try {
        ...
        import_core.elizaLogger.error("Failed to update goal:", {
          goalId: goal.id,
          error: error instanceof Error ? error.message : String(error),
          status: goal.status
        });
  ...
  async removeGoal(goalId) {
    ...
    return this.withDatabase(async () => {
      try {
        ...
        import_core.elizaLogger.debug("Goal removal attempt:", {
          goalId,
          removed: (result == null ? void 0 : result.rowCount) ?? 0 > 0
        });
      } catch (error) {
        import_core.elizaLogger.error("Failed to remove goal:", {
          goalId,
          error: error instanceof Error ? error.message : String(error)
        });
        throw error;
  ...
  async createRoom(roomId) {
    return this.withDatabase(async () => {
      const newRoomId = roomId || (0, import_uuid.v4)();
    ...
    return this.withDatabase(async () => {
      ...
      try {
        ...
        if (checkResult.rowCount === 0) {
          import_core.elizaLogger.warn("No room found to remove:", { roomId });
        ...
        await client.query("ROLLBACK");
        import_core.elizaLogger.error("Failed to remove room:", {
          roomId,
          error: error instanceof Error ? error.message : String(error)
        });
        throw error;
  ...
  async createRelationship(params) {
    ...
    return this.withDatabase(async () => {
      try {
        const relationshipId = (0, import_uuid.v4)();
        await this.pool.query(
        ...
        import_core.elizaLogger.debug("Relationship created successfully:", {
          relationshipId,
          userA: params.userA,
          userB: params.userB
        });
        ...
        if (error.code === "23505") {
          import_core.elizaLogger.warn("Relationship already exists:", {
            userA: params.userA,
            userB: params.userB,
            error: error instanceof Error ? error.message : String(error)
          });
        } else {
          import_core.elizaLogger.error("Failed to create relationship:", {
            userA: params.userA,
            userB: params.userB,
            error: error instanceof Error ? error.message : String(error)
          });
  ...
  async getRelationship(params) {
    ...
    return this.withDatabase(async () => {
      try {
        ...
        if (rows.length > 0) {
          import_core.elizaLogger.debug("Relationship found:", {
        ...
        import_core.elizaLogger.debug("No relationship found between users:", {
          userA: params.userA,
          userB: params.userB
        });
        return null;
      } catch (error) {
        import_core.elizaLogger.error("Error fetching relationship:", {
          userA: params.userA,
          userB: params.userB,
          error: error instanceof Error ? error.message : String(error)
        });
  ...
  async getRelationships(params) {
    ...
    return this.withDatabase(async () => {
      try {
        ...
        import_core.elizaLogger.debug("Retrieved relationships:", {
          userId: params.userId,
          count: rows.length
        });
        return rows;
      } catch (error) {
        import_core.elizaLogger.error("Failed to fetch relationships:", {
          userId: params.userId,
          error: error instanceof Error ? error.message : String(error)
        });
        throw error;
  ...
  async getCachedEmbeddings(opts) {
    ...
    return this.withDatabase(async () => {
      try {
        import_core.elizaLogger.debug("Fetching cached embeddings:", {
        ...
        import_core.elizaLogger.debug("Retrieved cached embeddings:", {
          count: rows.length,
          tableName: opts.query_table_name,
          matchCount: opts.query_match_count
        });
        ...
        import_core.elizaLogger.error("Error in getCachedEmbeddings:", {
          error: error instanceof Error ? error.message : String(error),
          tableName: opts.query_table_name,
          fieldName: opts.query_field_name
        });
  ...
  async log(params) {
    ...
    return this.withDatabase(async () => {
      try {
        ...
        import_core.elizaLogger.debug("Log entry created:", {
          logId,
          type: params.type,
          roomId: params.roomId,
          userId: params.userId,
        ...
        import_core.elizaLogger.error("Failed to create log entry:", {
          error: error instanceof Error ? error.message : String(error),
          type: params.type,
          roomId: params.roomId,
          userId: params.userId
  ...
  async searchMemoriesByEmbedding(embedding, params) {
    return this.withDatabase(async () => {
      import_core.elizaLogger.debug("Incoming vector:", {
        length: embedding.length,
        sample: embedding.slice(0, 5),
        isArray: Array.isArray(embedding),
        allNumbers: embedding.every((n) => typeof n === "number")
      });
      if (embedding.length !== (0, import_core.getEmbeddingConfig)().dimensions) {
        throw new Error(
          `Invalid embedding dimension: expected ${(0, import_core.getEmbeddingConfig)().dimensions}, got ${embedding.length}`
        );
      }
      ...
      import_core.elizaLogger.debug("Vector debug:", {
      ...
      let sql = `
                SELECT *,
                1 - (embedding <-> $1::vector(${(0, import_core.getEmbeddingConfig)().dimensions})) as similarity
                FROM memories
                WHERE type = $2
            `;
      const values = [vectorStr, params.tableName];
      import_core.elizaLogger.debug("Query debug:", {
        sql: sql.slice(0, 200),
      ...
      try {
        await this.pool.query(
          `INSERT INTO participants (id, "userId", "roomId")
                    VALUES ($1, $2, $3)`,
          [(0, import_uuid.v4)(), userId, roomId]
        );
        return true;
      } catch (error) {
  ...
  async getActorDetails(params) {
    ...
    return this.withDatabase(async () => {
      try {
        ...
        import_core.elizaLogger.debug("Retrieved actor details:", {
          roomId: params.roomId,
        ...
        import_core.elizaLogger.error("Failed to fetch actor details:", {
          roomId: params.roomId,
          error: error instanceof Error ? error.message : String(error)
        });
        throw new Error(
  ...
  async getCache(params) {
    return this.withDatabase(async () => {
      ...
      try {
        ...
        import_core.elizaLogger.error("Error fetching cache", {
          error: error instanceof Error ? error.message : String(error),
          key: params.key,
          agentId: params.agentId
        });
    ...
    return this.withDatabase(async () => {
      try {
        ...
        try {
          ...
          await client.query("ROLLBACK");
          import_core.elizaLogger.error("Error setting cache", {
            error: error instanceof Error ? error.message : String(error),
            key: params.key,
            agentId: params.agentId
          });
    ...
    return this.withDatabase(async () => {
      try {
        ...
        try {
          ...
          await client.query("ROLLBACK");
          import_core.elizaLogger.error("Error deleting cache", {
            error: error instanceof Error ? error.message : String(error),
            key: params.key,
            agentId: params.agentId
          });
  ...
  async removeKnowledge(id) {
    return this.withDatabase(async () => {
      ...
      try {
        ...
        await client.query("ROLLBACK");
        import_core.elizaLogger.error("Error removing knowledge", {
          error: error instanceof Error ? error.message : String(error),
          id
        });
        throw error;
